...
void PrintLogFile(void) {
	...
	fr = //Write your code here to open the log.txt file for read.
	if (fr != FR_OK) {
	...
	do {
		fr = //Write your code here to read the log.txt file.
		if (fr != FR_OK) {
...
void LogTask(void *pvParameters) {
	...
	xSemaphoreTake(fatfsMutex, portMAX_DELAY);
	fr = //Write your code here to open and clear the log.txt file. If the file is not exists, create a new one.
	if (fr == FR_OK) {
		//Write your code here to close the log.txt file.
	...
	for(;;){
		if (xQueueReceive(logQueue, &logMsg, portMAX_DELAY) == pdPASS) {
			xSemaphoreTake(fatfsMutex, portMAX_DELAY);
			if(!fileOpened) {
				fr = //Write your code here to open the log.txt file for write.
				if (fr != FR_OK){
			...
			fr = //Write your code here to write to the log.txt file.
			if (fr == FR_OK){
				f_sync(&logFile);
