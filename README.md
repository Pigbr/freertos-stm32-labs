...
5 個漸進式 lab,從基本 task + queue 入門,中間直接走 FreeRTOS kernel 的 TCB linked list、改 heap memory management,最後做出完整的音樂播放器(RTOS + FATFS + SD card + I²S DAC + UART log + 按鍵狀態機)。所有 lab 跑在 STM32F407VG Discovery Board,用 STM32CubeIDE + STM32CubeMX 產生底層 HAL。
...
- Audio playback 透過 CS43L22 DAC over I²S
...
遇到的問題:按了按鈕卻要等紅橘綠燈閃完才會切換 → 用 `goto` 跳出 LED loop、立刻處理切換。另外用一個變數記住當前閃到哪顆燈,切換後不從頭開始。
...
遇到的問題:list item 的 `pvOwner` 是 `void *`,一開始不知道要 cast 成什麼。查資料後發現要 cast 成 `TCB_t *` 才能解出每個欄位。
...
遇到的問題:照 slide 配置完中斷一直停在高電位 → 查資料後發現少配置一個 register(`outs_REG` 讀完才會清中斷源)。

練到:用 binary semaphore 把 ISR 的工作交給 task 處理、`...FromISR` API 配 yield、SPI sensor 配置。
...
遇到的問題:直接拿實驗給的 `heap_2.c` 整檔覆蓋,編譯過但 STM32 reset 後卡死。後來才知道 FreeRTOS 版本不同會掛,要對照範本手動 patch 自己版本的檔案。

練到:動態記憶體配置的內部運作、fragmentation 與 coalescing、版本管理意識。
...
| 狀態機映射 | 不同 system state 下,同樣的按鈕事件對應不同操作 |
...
從 Lab 1 的「按鈕亮燈」一路到 Lab 5 的「按鈕狀態機整合進完整系統」,看得到自己對 RTOS 架構應用越來越熟。
